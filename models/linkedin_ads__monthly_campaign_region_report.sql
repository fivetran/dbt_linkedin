...
report as (
    ...
    select *,
        ...
        {% endif %}
    from {{ var('monthly_ad_analytics_by_member_region') }}
),
...
final as (
    ...
    select 
        ...
        {{ linkedin_ads_persist_pass_through_columns(pass_through_variable='linkedin_ads__conversion_fields', transform='sum', coalesce_with=0, except_variable='linkedin_ads__monthly_ad_analytics_by_member_region_passthrough_metrics', exclude_fields=['conversion_value_in_local_currency']) }}
        
        {{ linkedin_ads_persist_pass_through_columns(pass_through_variable='linkedin_ads__monthly_ad_analytics_by_member_region_passthrough_metrics', transform='sum', exclude_fields=['conversion_value_in_local_currency']) }}

    from report 
    left join geo
        on geo.geo_id = report.member_region
